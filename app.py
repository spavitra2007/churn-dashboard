...
def make_gauge(score, color):
    fig = go.Figure(go.Indicator(
        ...
        gauge={
            'axis': {'range': [None, 100], 'tickwidth': 1, 'tickcolor': "white", 'showticklabels': False},
...
page = st.sidebar.radio(
    "NAVIGATION",
    ("Analysis", "Recommendation System", "Feature Engineering Models", "Retention System")
)
...
if page == "Analysis":
    ...
    total_rev = df_pred['loan_outstanding_amount'].sum() if 'loan_outstanding_amount' in df_pred else 0
    churn_rate = df_pred['Predicted'].mean() * 100 if 'Predicted' in df_pred else 0
    avg_rev = total_rev / total_cust if total_cust > 0 else 0
    active_users = df_pred['mobile_banking_active_flag'].sum() if 'mobile_banking_active_flag' in df_pred else 0
    ...
    with r1c1:
        ...
        if 'tenure_months' in df_pred and 'credit_card_limit' in df_pred:
            area_data = df_pred.groupby('tenure_months')['credit_card_limit'].mean().reset_index()
            fig1 = px.area(area_data, x='tenure_months', y='credit_card_limit', color_discrete_sequence=['#a855f7'])
            fig1.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=350, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
    ...
    with r1c2:
        ...
        if 'Risk_Segment' in df_pred:
            ...
            fig2 = px.pie(donut_data, values='Count', names='Risk_Segment', hole=0.6, 
                          color_discrete_sequence=['#8b5cf6', '#3b82f6', '#10b981', '#f59e0b', '#d1d5db'])
            fig2.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=350, showlegend=True, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
    ...
    with r2c1:
        ...
        if 'age_group' in df_pred:
            ...
            fig3 = px.bar(bar_data, x='Age Group', y='Count', color_discrete_sequence=['#3b82f6'])
            fig3.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=350, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig3, use_container_width=True)

    with r2c2:
        st.subheader("Churn Probability by Tenure")
        if 'tenure_months' in df_pred and 'Churn_Probability' in df_pred:
            area_churn = df_pred.groupby('tenure_months')['Churn_Probability'].mean().reset_index()
            fig4 = px.area(area_churn, x='tenure_months', y='Churn_Probability', color_discrete_sequence=['#ec4899'])
            fig4.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=350, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
    ...
    with r3c1:
        ...
        if 'occupation_type' in df_pred and 'Churn_Probability' in df_pred:
            occ_data = df_pred.groupby('occupation_type')['Churn_Probability'].mean().sort_values().reset_index()
            fig5 = px.bar(occ_data, x='Churn_Probability', y='occupation_type', orientation='h', color_discrete_sequence=['#f43f5e'])
            fig5.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=350, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig5, use_container_width=True)
            
    with r3c2:
        st.subheader("Income vs Age (Risk Map)")
        if 'age' in df_pred and 'annual_income' in df_pred and 'Risk_Segment' in df_pred:
            fig6 = px.scatter(df_pred, x='age', y='annual_income', color='Risk_Segment', opacity=0.6,
                              color_discrete_map={'High': '#ef4444', 'Medium': '#f59e0b', 'Low': '#10b981'})
            fig6.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=350, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
    ...
    if df_preds2.empty:
        ...
        if 'customer_id' not in df_preds2.columns and 'customer_id' in df_recos.columns:
            df_preds2['customer_id'] = df_recos['customer_id'].iloc[:len(df_preds2)].values
        ...
        if 'customer_id' in df_preds2.columns and 'customer_id' in df_recos.columns:
            ...
            cust_display = {c: f"Customer Profile {i+1}" for i, c in enumerate(cust_list)}
            ...
            if selected_cust:
                ...
                with col1:
                    ...
                    st.markdown(f"**Marital Status:** {cust_data.get('marital_status', 'N/A')}")
                    st.markdown(f"**Education:** {cust_data.get('education_level', 'N/A')}")
                    st.markdown(f"**Occupation:** {cust_data.get('occupation_type', 'N/A')}")
                ...
                with col2:
                    ...
                    st.markdown(f"**Annual Income:** ₹ {income:,.2f}" if pd.notnull(income) else "**Annual Income:** N/A")
                    st.markdown(f"**Credit Score:** {cust_data.get('credit_score', 'N/A')}")
                    st.markdown(f"**Tenure (Months):** {cust_data.get('tenure_months', 'N/A')}")
                    ...
                    st.markdown(f"**Risk Segment:** <span style='color: {risk_color}; font-weight: bold;'>{risk} Risk</span>", unsafe_allow_html=True)
                    
                    st.markdown(f"<p style='margin-bottom: 5px;'><b>Churn Probability:</b> {churn_prob:.1f}%</p>", unsafe_allow_html=True)
                ...
                with col3:
                    ...
                    if not cust_reco.empty:
                        reco_prod = cust_reco.iloc[0].get('recommended_next_product', 'No Recommendation Available')
                        pretty_prod = str(reco_prod).replace('_flag', '').replace('_', ' ').title()
                        ...
                        for col in flags:
                            if col in cust_reco.columns and cust_reco.iloc[0][col] == 1:
                                held.append(col.replace('_flag', '').replace('_', ' ').title())
                        
                        if held:
                            st.markdown(f"<div style='display: flex; flex-wrap: wrap; gap: 8px;'>", unsafe_allow_html=True)
                            for h in held:
                                st.markdown(f"<span style='background-color: #e5e7eb; padding: 4px 10px; border-radius: 15px; font-size: 0.85em; color: #374151;'>✔️ {h}</span>", unsafe_allow_html=True)
                            st.markdown("</div>", unsafe_allow_html=True)
                        else:
                ...
                # Visuals for Context
                st.markdown("<br><h4 style='color: #334155;'>📊 Distribution Context</h4>", unsafe_allow_html=True)
                vc1, vc2 = st.columns(2)
                with vc1:
                    # Income distribution with customer position
                    if 'annual_income' in df_preds2.columns:
                        fig_inc = px.histogram(df_preds2, x='annual_income', nbins=30, color_discrete_sequence=['#3b82f6'])
                        fig_inc.add_vline(x=income, line_dash="dash", line_color="#ef4444", annotation_text="★ This Customer", annotation_position="top right")
                        fig_inc.update_layout(title="Annual Income Distribution (Across all customers)", height=280, margin=dict(l=0, r=0, t=35, b=0), plot_bgcolor='rgba(0,0,0,0)')
                        st.plotly_chart(fig_inc, use_container_width=True)
                with vc2:
                    if 'credit_score' in cust_data and 'credit_score' in df_preds2.columns:
                        fig_cs = px.histogram(df_preds2, x='credit_score', nbins=30, color_discrete_sequence=['#10b981'])
                        fig_cs.add_vline(x=cust_data['credit_score'], line_dash="dash", line_color="#ef4444", annotation_text="★ This Customer", annotation_position="top left")
                        fig_cs.update_layout(title="Credit Score Distribution (Across all customers)", height=280, margin=dict(l=0, r=0, t=35, b=0), plot_bgcolor='rgba(0,0,0,0)')
                        st.plotly_chart(fig_cs, use_container_width=True)
        else:
            st.error("The required 'customer_id' column was not found in both datasets.")
    ...
    if df.empty:
        ...
        models_config = [
            {"num": "1", "name": "1.Churn early warning signal", "desc": "How engaged is the customer across key touchpoints", "score": 72, "color": "#2563eb", "trend": [65, 68, 72, 78, 74, 72], "type": "line", "insights": ["Engagement improving till Apr, slight dip in May-Jun", "Below peak engagement - monitor closely"]},
            {"num": "2", "name": "2.02 Financial stress index", "desc": "What is the customer value and revenue potential", "score": 68, "color": "#16a34a", "trend": [80, 85, 90, 95, 88, 82], "type": "bar", "insights": ["Consistent revenue contributor", "Minor dip in last 2 months"]},
            {"num": "3", "name": "3.03 Digital maturity score", "desc": "How satisfied is the customer with product/service", "score": 63, "color": "#9333ea", "trend": [70, 68, 65, 62, 60, 63], "type": "line", "insights": ["Satisfaction gradually declining", "Recent slight improvement - early positive sign"]},
            {"num": "4", "name": "4.04 Life stage profile", "desc": "What do customer actions and usage patterns indicate", "score": 58, "color": "#f97316", "trend": [80, 78, 72, 65, 60, 58], "type": "line", "insights": ["Usage consistently declining", "Drop in active days and feature usage"]},
            {"num": "5", "name": "05 Trust and experience gap", "desc": "How customer interacts with support and issue resolution", "score": 62, "color": "#0891b2", "trend": [30, 40, 45, 42, 40, 46], "type": "bar", "insights": ["Increasing tickets raised", "Resolution rate stable but volume high"]},
            {"num": "6", "name": "06 Competitive pressure signal", "desc": "What is the overall risk based on multiple risk signals", "score": 70, "color": "#e11d48", "trend": [90, 85, 80, 75, 78, 82], "type": "line", "insights": ["Multiple high risk signals detected", "Overall risk is elevated"]}
        ]
        ...
        for i in range(0, len(models_config), 2):
            ...
            for j, col in enumerate(cols):
                if i + j < len(models_config):
                    m = models_config[i+j]
                    with col:
                        st.markdown(f"**<span style='color: white; background-color: {m['color']}; padding: 4px 12px; border-radius: 6px; margin-right: 12px; font-size: 1.1em;'>{m['num']}</span> <span style='color: {m['color']}; font-size: 1.1em;'>{m['name'].upper()}</span>**", unsafe_allow_html=True)
                        st.markdown(f"<p style='color: #64748b; margin-top: 5px; margin-bottom: 15px;'>{m['desc']}</p>", unsafe_allow_html=True)
                        
                        gc1, gc2 = st.columns([1, 1.2])
                        with gc1:
                            fig_gauge = make_gauge(m["score"], m["color"])
                            fig_gauge.update_layout(height=180, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
                            st.plotly_chart(fig_gauge, use_container_width=True)
                        with gc2:
                            st.markdown("<p style='color: #94a3b8; font-size: 0.8em; margin-bottom: 0;'>Trend (Last 6 Months)</p>", unsafe_allow_html=True)
                            trend_df = pd.DataFrame({'x': trend_x, 'y': m['trend']})
                            fig_spark = make_sparkline(trend_df, m["color"], m["type"])
                            fig_spark.update_layout(height=160, plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)')
                            st.plotly_chart(fig_spark, use_container_width=True)
                        
                        st.markdown(f"<span style='color: {m['color']}; font-size: 1em; font-weight: bold;'>Key Insights</span>", unsafe_allow_html=True)
                        for insight in m["insights"]:
                            st.markdown(f"- <span style='font-size: 0.9em; color: #475569;'>{insight}</span>", unsafe_allow_html=True)

        st.markdown("---")
        
        # Final Consolidated Section
        st.markdown("<h3 style='text-align: center; background-color: #f8fafc; padding: 10px; border-radius: 8px;'>FINAL CHURN INSIGHT – CONSOLIDATED FROM ALL 6 MODELS</h3>", unsafe_allow_html=True)
        ...
        with fc2:
            ...
            overall_churn = int(df['Churn_Probability'].mean() * 100) if 'Churn_Probability' in df else 72
            fig_final = make_gauge(overall_churn, "#ef4444")
            fig_final.update_layout(height=180)
            st.plotly_chart(fig_final, use_container_width=True)
            st.markdown(f"<h3 style='text-align: center; color: #ef4444; margin-top: -30px;'>HIGH RISK</h3>", unsafe_allow_html=True)
        ...
        st.markdown("<div style='text-align: center; color: #ef4444; font-weight: bold; margin-top: 15px;'>💡 Proactive action can reduce churn risk and improve customer retention & lifetime value.</div>", unsafe_allow_html=True)
    ...
    if df_seg.empty:
        ...
        st.markdown(f"### ⚠️ Action Center: {len(high_risk)} High-Risk Profiles (Actual Churn = 0)")
        ...
        if selected_cust is not None:
            ...
            m1.metric("Financial Stress Index (FSI)", f"{float(fsi):.1f} / 100", "- Critical", delta_color="inverse")
            m2.metric("Digital Engagement", f"{float(engagement):.0f} / 100", "- Dropping", delta_color="inverse")
            m3.metric("AI Churn Prediction", f"{float(churn_prob):.1f}%", "+ Immediate Risk", delta_color="inverse")
            ...
            st.markdown("---")
            st.markdown("<h2 style='text-align: center; color: #1e293b; margin-bottom: 25px;'>🛠️ The 6-Pillar Retention Playbook</h2>", unsafe_allow_html=True)
